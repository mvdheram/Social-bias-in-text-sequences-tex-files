...
\caption{This table shows two samples, each belonging to  Intersentence and intrasentence with corresponding contextual sentence describing the context, domain describing the bias type and target types describing the target the sample belongs to, the intent is to give details regarding the creation of sample types}
\label{tab:stereoset}
\end{table}
...
\caption{CrowS-Pairs stereotype example pair where stereo in "stereo\_antistereo" category indicates that the sent\_more is more stereotypical and sent\_less violates the stereotype}
\label{tab:Crowspair}
